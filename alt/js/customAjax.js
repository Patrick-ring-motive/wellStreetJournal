window.dev.null=async function(){};
...
var req;
  ...
  if(request.search('api.wsj.net')>-1){return dev.null;}
  if(request.search('wsjstream.wsj.net')>-1){return dev.null;}
    req=new Request(request,headers);
    response = await window.nativeFetch(req);
  
  }else{
  if(request.url.search('api.wsj.net')>-1){return dev.null;}
  if(request.url.search('wsjstream.wsj.net')>-1){return dev.null;}
